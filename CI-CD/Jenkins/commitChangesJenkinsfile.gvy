            ...
            script {
                catchError(buildResult: 'SUCCESS', stageResult: 'FAILURE') {
                    withCredentials([usernamePassword(credentialsId: 'github', passwordVariable: 'GIT_PASSWORD', usernameVariable: 'GIT_USERNAME')]) {
                        ...
                        sh "cat deploy/backend-server/backend-server-deployment.yaml"
                        sh "sed -i 's+aman55/articlearena.*+aman55/articlearena:${DOCKERTAG}+g' deploy/backend-server/backend-server-deployment.yaml"
                        sh "cat deploy/backend-server/backend-server-deployment.yaml"
                        sh "git add ."
